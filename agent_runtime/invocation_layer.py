...
class AgentInvocationLayer:
    ...
    def invoke(
        ...
        if mode == InvocationMode.AUTOMATED:
            adapter_outputs = self._run_automated(adapter, input_paths, ctx.artifacts_dir, contract, schemas)
    ...
    def _resolve_inputs(self, ctx: RunContext, contract: AgentContract) -> dict[str, Path]:
        """
        Resolve each declared input artifact to a filesystem path.

        Lookup order per artifact name:
        1. ctx.artifacts_dir   — run-specific artifacts (e.g. change_intent.yaml after init)
        2. ctx.project_inputs_root — project input files (e.g. domain_scope.md, glossary.md)

        This two-location lookup is necessary because project inputs live in the project root
        while run-specific artifacts live in the run directory.

        Raises MissingInputArtifactError if an input file is not found in either location.
        """
        ...
        for name in contract.input_artifacts:
            ...
            if path.is_file():
                ...
                continue
            if ctx.project_inputs_root is not None:
                project_path = ctx.project_inputs_root / name
                if project_path.is_file():
                    input_paths[name] = project_path
                    continue
            raise MissingInputArtifactError(
                f"Required input artifact missing: '{name}' not found in "
                f"{ctx.artifacts_dir} or {ctx.project_inputs_root}."
            )
        return input_paths
    ...
    def _run_automated(
        ...
        if adapter is None:
            adapter = self._build_default_adapter(contract, schemas or {})
    ...
    def _build_default_adapter(
        ...
        """
        Create an LLMAgentAdapter from environment configuration.

        Raises MissingAdapterError if AUTOMATED mode is requested but the LLM environment
        variables (DEVOS_LLM_API_URL, DEVOS_LLM_API_KEY) are not set.
        """
        try:
            from agent_runtime.llm_adapter import LLMAgentAdapter  # noqa: PLC0415
            from agent_runtime.llm_client import LLMClientError, llm_client_from_env  # noqa: PLC0415

            client = llm_client_from_env()
        except Exception as exc:  # noqa: BLE001
            llm_error_types: tuple[type, ...] = ()
            try:
                from agent_runtime.llm_client import LLMClientError as _LLMClientError  # noqa: PLC0415
                llm_error_types = (_LLMClientError,)
            except ImportError:
                pass
            if llm_error_types and isinstance(exc, llm_error_types):
                raise MissingAdapterError(
                    "AUTOMATED mode requires either an explicit adapter or LLM environment "
                    f"variables. Missing configuration: {exc}"
                ) from exc
            raise MissingAdapterError(
                f"AUTOMATED mode: failed to build default LLM adapter: {exc}"
            ) from exc
        return LLMAgentAdapter(  # type: ignore[return-value]
