...
Rules enforced (rule_id → source section):
  ...
  STORY_MISSING_TEST_TASK At least one task in the story must have a name containing
                          ...
                          Implementation and verification must be represented by separate
                          tasks to produce explicit, attributable evidence.
...
def _lint_story(
    ...
    # design_freedom validity
    if story.design_freedom is not None:
        normalized = story.design_freedom.strip().lower()
        if normalized not in _STORY_DESIGN_FREEDOM_VALUES:
            violations.append(LintViolation(
                context=context,
    ...
    if task_count >= _STORY_MIN_TASKS and not _story_has_test_task(story):
        violations.append(LintViolation(
            ...
            message=(
                "Story contains no task with a test-oriented name. "
                f"At least one task name must contain a keyword from: "
                f"{sorted(_TEST_TASK_KEYWORDS)}. "
...
def _story_has_test_task(story: StoryModel) -> bool:
    """
    Return True if at least one task in the story has a name containing a
    test-oriented keyword.

    Detection is case-insensitive substring matching against a fixed keyword set.
