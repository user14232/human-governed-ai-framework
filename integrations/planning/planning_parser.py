"""
YAML parser for DevOS planning artifacts.
"""
...
def parse_planning_yaml(path: str) -> ProjectModel:
...
def _parse_project(raw: dict[str, Any], errors: list[str]) -> ProjectModel | None:
    ...
    milestones = tuple(_parse_milestone(m, idx, errors) for idx, m in enumerate(milestones_raw))
    ...
    epics = tuple(_parse_epic(e, idx, errors) for idx, e in enumerate(epics_raw))
...
def _parse_milestone(raw: Any, idx: int, errors: list[str]) -> MilestoneModel | None:
    ...
    return MilestoneModel(name=name, description=description, target_date=_opt_str(raw, "target_date"))
...
def _parse_epic(raw: Any, idx: int, errors: list[str]) -> EpicModel | None:
    ...
    stories = tuple(_parse_story(s, idx, s_idx, errors) for s_idx, s in enumerate(stories_raw))
...
def _parse_story(
    ...
    effort = _require_bounded_int(
        raw, "effort", context, errors, lo=_EFFORT_COMPLEXITY_MIN, hi=_EFFORT_COMPLEXITY_MAX
    )
    complexity = _require_bounded_int(
        raw, "complexity", context, errors, lo=_EFFORT_COMPLEXITY_MIN, hi=_EFFORT_COMPLEXITY_MAX
    )
    ...
    if effort is not None and complexity is not None:
        ...
        if explicit is not None and explicit != computed_estimate:
            errors.append(
                ...
                "Remove 'estimate' and let it be derived automatically."
            )
            estimate = None
        else:
    ...
    tasks = tuple(
        _parse_task(t, epic_idx, story_idx, t_idx, errors) for t_idx, t in enumerate(tasks_raw)
    )
...
def _parse_task(
    ...
    errors.append(
        f"{context}: task must be a string or a mapping with 'name', got {type(raw).__name__}."
    )
    return None
...
def _require_str(mapping: dict[str, Any], key: str, context: str, errors: list[str]) -> str | None:
    value = mapping.get(key)
    if value is None:
        errors.append(f"{context}: required field '{key}' is missing.")
        return None
    if not isinstance(value, str):
        errors.append(f"{context}: field '{key}' must be a string, got {type(value).__name__}.")
        return None
    if not value.strip():
...
def _opt_int(mapping: dict[str, Any], key: str, context: str, errors: list[str]) -> int | None:
    value = mapping.get(key)
    if value is None:
        return None
    ...
    if isinstance(value, int):
        return value
    errors.append(f"{context}: field '{key}' must be an integer, got {type(value).__name__}.")
    return None
...
def _opt_float(
    ...
    if isinstance(value, (int, float)):
        return float(value)
    errors.append(f"{context}: field '{key}' must be numeric, got {type(value).__name__}.")
    return None
...
def _opt_str_list(
    ...
    for i, item in enumerate(value):
        if not isinstance(item, str):
            errors.append(f"{context}: {key}[{i}] must be a string, got {type(item).__name__}.")
        elif item.strip():
            result.append(item.strip())
    return tuple(result)


def _opt_bool(mapping: dict[str, Any], key: str, context: str, errors: list[str]) -> bool | None:
    value = mapping.get(key)
    if value is None:
        return None
    if isinstance(value, bool):
        return value
    errors.append(f"{context}: field '{key}' must be a boolean, got {type(value).__name__}.")
    return None
...
def _require_bounded_int(
    ...
    value = mapping.get(key)
    if value is None:
        errors.append(f"{context}: required field '{key}' is missing (integer {lo}-{hi} required).")
        return None
    if isinstance(value, bool):
        errors.append(f"{context}: field '{key}' must be an integer {lo}-{hi}, not a boolean.")
        return None
    if not isinstance(value, int):
        errors.append(
            f"{context}: field '{key}' must be an integer {lo}-{hi}, got {type(value).__name__}."
        )
        return None
    if not (lo <= value <= hi):
        errors.append(
            f"{context}: field '{key}' must be between {lo} and {hi} inclusive, got {value}."
        )
        return None
    return value
